...
r = 10 #Radius of curvature of the lens
H = 3 #Height of the lens (thickness if you want)
L = 7.14 #Lenght of triangle formed (wideness of the lens)
...
with open('lenteV1.txt', 'w') as f: #Open the file in order to write the GCODE to it
    ...
    for i in range(int(H/DOF)):
        ...
        print("Distancia:", round(abs(longitud_pasada-L), 2), "   ", round(pasada, 2)) #imprimo el valor; solo para demostracion
        f.write("G1 "+ "Y-" + str(round(pasada,2))) #Go on Y direction (depth of pass)
        f.write('\n')
        f.write("G1 " + "X" + str(round(abs(longitud_pasada-L), 2))) #Go on X direction (shape of lens)
        f.write('\n')
        f.write("G0 X"+ str(Z))
        f.write('\n')
